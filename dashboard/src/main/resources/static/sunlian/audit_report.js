            ...
            function updateTable(data){
                ...
                data.forEach(function(ii){
                    var row="<tr><td><input type='checkbox' name='id' value='"+ ii.id +"' ></td>";
                    for(var item in ii){
			...
			function submitQueryForm(formID,HTTPMethod) {
				...
				$.ajax({
					...
					data: submitdata,
                        ...
                        crossDomain: true ,
					success: function(data) {
					    console.log("submitQueryForm:rows"+data);
						totalPage=Math.ceil(data/5);
                        console.log(totalPage);
                        //生成页码标记
						...
						$.ajax({
							...
							type: HTTPMethod ,
							data: formdata,
							success: function(data) {console.log("submitQueryForm:data"+data);updatePage();updateTable(data);},
							error: function(code, message, details) {
								console.log(message);
								console.log(code);
								console.log(details);
